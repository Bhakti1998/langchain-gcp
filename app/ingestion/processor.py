...
def _ensure_collection():
    if not qdrant_client.collection_exists(QDRANT_COLLECTION_name):
        ...
        logfire.info(f"Created Qdrant collection '{QDRANT_COLLECTION_name}'")
    ...
    with logfire.span("Processing File", file=filename, source=source_type,
        ...
        try:
            ...
            if not skip_raw_upload:
                upload_to_gcs(file_path, raw_bucket, raw_gcs_path)
            else:
                logfire.info(f"Skipping RAW upload (cloud mode) — file already at gs://{raw_bucket}/{raw_gcs_path}")
            ...
            # Step 3: Chunk -> full_text is raw data in readable format
            ...
            upload_to_gcs(processed_data, processed_bucket, processed_gcs_path, is_json=True)
                ...
                points = [
                    models.PointStruct(
                        ...
                        payload={
                            ...
                            "raw_gcs_path": f"gs://{raw_bucket}/{raw_gcs_path}",
                        },
                    )
        ...
        if wipe:
            with logfire.span("Wiping Collection"):
                if qdrant_client.collection_exists(QDRANT_COLLECTION_name):
                    ...
                    logfire.info(f"Collection '{QDRANT_COLLECTION_name}' deleted")
